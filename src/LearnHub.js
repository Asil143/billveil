  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050810", fontFamily: FONT, color: "#f1f5f9" }}>
      ...
      <div style={{ background: "rgba(5,8,16,0.95)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "14px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 600, color: "#64748b" }}>Learn</span>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 20px 80px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          <h1 style={{ fontSize: "clamp(28px, 6vw, 48px)", fontWeight: 900, lineHeight: 1.1, letterSpacing: "-0.03em", marginBottom: 16, color: "#f1f5f9" }}>
            Learn by seeing it{" "}
            <span style={{ color: "#10b981", textShadow: "0 0 24px rgba(16,185,129,0.35)" }}>happen to someone</span>
          </h1>
          <p style={{ fontSize: "clamp(15px, 3vw, 18px)", color: "#64748b", lineHeight: 1.7, maxWidth: 560, margin: "0 auto 32px" }}>
            Every story is a real situation — explained step by step, with exactly what the person did to fight back and win.
          </p>
          ...
          <div style={{ display: "inline-flex", gap: 0, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, overflow: "hidden" }}>
            {[
              { n: `${live}`, label: "Live stories" },
              { n: totalSaved, label: "Documented savings" },
              { n: `${stories.length}`, label: "Stories total" },
            ].map(({ n, label }, i) => (
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginBottom: 36 }}>
          {FILTERS.map(f => (
            <button
              ...
              style={{
                padding: "7px 16px",
                background: filter === f ? "rgba(16,185,129,0.12)" : "rgba(255,255,255,0.04)",
                border: `1px solid ${filter === f ? "rgba(16,185,129,0.4)" : "rgba(255,255,255,0.08)"}`,
                borderRadius: 20,
        ...
        <div className="learn-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
          {visible.map(story => (
            <StoryCard key={story.slug} story={story} onClick={() => story.status === "live" && router.push(`/learn/${story.slug}`)} />
          ))}
        </div>
...
function StoryCard({ story, onClick }) {
  ...
  return (
    <div
      className={isLive ? "learn-card" : ""}
      onClick={onClick}
      style={{
        background: "rgba(255,255,255,0.03)",
        border: `1px solid ${isLive ? "rgba(255,255,255,0.09)" : "rgba(255,255,255,0.05)"}`,
        borderTop: `3px solid ${isLive ? story.color : "rgba(255,255,255,0.06)"}`,
        borderRadius: 16,
        padding: 22,
    ...
    >
      ...
      {isLive && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: story.color }}>Read story →</span>
        </div>
      )}
    </div>
  );
