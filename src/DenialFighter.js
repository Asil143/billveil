...
export default function DenialFighter() {
  ...
  const parseResult = (text) => {
    ...
    return sections.map((section, i) => {
      ...
      const verdictBg = content.includes("LIKELY INVALID") ? "rgba(52,211,153,0.08)"
        : content.includes("POSSIBLY INVALID") ? "rgba(251,191,36,0.08)" : "rgba(239,68,68,0.08)";
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24, marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 10, marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {EXAMPLES.map((ex) => (
              <button key={ex} onClick={() => setDenial(ex)} style={{ padding: "5px 12px", background: denial === ex ? "rgba(16,185,129,0.12)" : "rgba(255,255,255,0.04)", color: denial === ex ? "#10b981" : "#64748b", border: `1px solid ${denial === ex ? "rgba(16,185,129,0.3)" : "rgba(255,255,255,0.08)"}`, borderRadius: 24, fontSize: 12, fontWeight: 500, cursor: "pointer", transition: "all 0.15s", fontFamily: FONT }}>
                {ex}
              </button>
            ))}
          </div>
        ...
        <button
          ...
          style={{ width: "100%", padding: "15px", background: loading || !denial.trim() ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg, #10b981, #059669)", color: loading || !denial.trim() ? "#334155" : "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: loading || !denial.trim() ? "default" : "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: FONT, boxShadow: loading || !denial.trim() ? "none" : "0 8px 28px rgba(16,185,129,0.35)" }}
        >
          {loading ? (
            <>
        ...
        <div>
          ...
          <div style={{ marginTop: 28, background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.15)", borderRadius: 16, padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div onClick={() => { copyAppeal(); markDone("appeal"); }} style={{ background: actionsDone.appeal ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.03)", border: `1px solid ${actionsDone.appeal ? "rgba(16,185,129,0.3)" : "rgba(255,255,255,0.08)"}`, borderRadius: 12, padding: "16px 18px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} style={{ flex: 1, minWidth: 180, padding: "10px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: selectedState ? "#f1f5f9" : "#475569", fontSize: 13, fontFamily: FONT, outline: "none", cursor: "pointer" }}>
                    <option value="">Select your state...</option>
                    {STATES.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                  {selectedState && (
                    <a href={getCommissionerUrl(selectedState)} target="_blank" rel="noopener noreferrer" onClick={() => markDone("state")} style={{ padding: "10px 16px", background: "linear-gradient(135deg, #10b981, #059669)", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap", display: "flex", alignItems: "center" }}>
                      Find Portal →
                    </a>
                  )}
                </div>
              ...
              <a href="https://www.consumerfinance.gov/complaint/" target="_blank" rel="noopener noreferrer" onClick={() => markDone("cfpb")} style={{ textDecoration: "none", display: "block" }}>
              ...
              <a href="https://www.patientadvocate.org/connect-with-services/find-a-case-manager/" target="_blank" rel="noopener noreferrer" onClick={() => markDone("advocate")} style={{ textDecoration: "none", display: "block" }}>
                <div style={{ background: actionsDone.advocate ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.03)", border: `1px solid ${actionsDone.advocate ? "rgba(16,185,129,0.3)" : "rgba(255,255,255,0.08)"}`, borderRadius: 12, padding: "16px 18px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#64748b" }}>Patient Advocate Foundation provides free case managers who fight insurance denials for you.</div>
                  </div>
